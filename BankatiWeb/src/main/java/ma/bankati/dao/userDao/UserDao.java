...
public class UserDao implements IUserDao {
    ...
    private User map(String fileLine){
        try {
            ...
            if (fields.length < 7) {
                System.err.println("Ligne invalide: " + fileLine);
                return null;
            }
            ...
            if (!fields[6].equals("null")) {
                try {
                    ...
                    System.err.println("Erreur de format de date: " + fields[6]);
                    dateCreation = LocalDate.now(); // Date par défaut
                }
            }

            return new User(id, firstName, lastName, username, password, role, dateCreation);
        } catch (Exception e) {
            System.err.println("Erreur lors du parsing de la ligne: " + fileLine);
            e.printStackTrace();
            return null;
        }
    }
    ...
    private String mapToFileLine(User user){
        ...
        String  roleStr     = user.getRole()      == null ? "null" : user.getRole().toString();
        String  creationDate  = user.getCreationDate()  == null  ? "null" : user.getCreationDate().format(DateTimeFormatter.ofPattern("yyyy/MM/dd"));


        return id+"-"+firstName+"-"+lastName+"-"+username+"-"+password+"-"+roleStr+"-"+creationDate + System.lineSeparator();
    ...
    public List<User> findAll() {
        try {
            return Files.readAllLines(path)
                    ...
                    .skip(1)
                    .map(line -> map(line))
                    .filter(user -> user != null) // Filtrer les lignes invalides
                    .toList() ;
        }
        catch (IOException e) {
